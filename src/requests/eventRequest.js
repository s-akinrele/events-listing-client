...
export const registerForEvent = data => dispatch => {
  dispatch(asyncActions(REGISTER_EVENT).loading(true))

  axios.post(eventsConstant.REGISTER_EVENT, data, {headers: {Authorization: token()}})
    .then(response => {
      dispatch(asyncActions(REGISTER_EVENT).success(response.data))
      dispatch(asyncActions(REGISTER_EVENT).loading(false))
    })
    .catch(error => {
      dispatch(asyncActions(REGISTER_EVENT).failure(true, error.response.data.message))
      dispatch(asyncActions(REGISTER_EVENT).loading(false))
    });
}